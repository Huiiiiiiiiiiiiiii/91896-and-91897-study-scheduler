...
class User:
    ...
    # ===== 新增：任务管理方法 =====
    def add_task(self, title, subject, deadline=None, priority="Medium"):
...
class MainApp:
    ...
    def update_display(self):
        ...
        for name, data in self.user.subjects.items():
            ...
            self.subject_list.insert(
                tk.END,
                f"{name}  {data['hours']}h/wk  [{data['priority']}]{deadline_str}\n"
            )
    ...
    def update_stats(self):
        ...
        self.stats_display.insert(tk.END, f"Tasks: {self.user.get_completed_count()}/{self.user.get_task_count()} completed\n\n")
        
        self.stats_display.insert(tk.END, "--- Subject Breakdown ---\n")
        for name, data in self.user.subjects.items():
            self.stats_display.insert(tk.END, f"{name}: {data['hours']}h/week  [{data['priority']}]\n")
    ...
    def setup_tasks_tab(self):
        ...
        tk.Label(row1, text="Task Title:", bg=COLORS["card_bg"], fg=COLORS["text"], font=("Helvetica", 10)).pack(side="left", padx=5)
        self.task_title_entry = tk.Entry(row1, font=("Helvetica", 11), fg=COLORS["text"], bg=COLORS["entry_bg"], width=25)
        self.task_title_entry.pack(side="left", padx=5)
        
        row2 = tk.Frame(add_card, bg=COLORS["card_bg"])
        row2.pack(fill="x", pady=2)
        tk.Label(row2, text="Subject:", bg=COLORS["card_bg"], fg=COLORS["text"], font=("Helvetica", 10)).pack(side="left", padx=5)
        ...
        tk.Label(row2, text="(optional)", bg=COLORS["card_bg"], fg=COLORS["text_light"], font=("Helvetica", 9)).pack(side="left")
        
        row3 = tk.Frame(add_card, bg=COLORS["card_bg"])
        row3.pack(fill="x", pady=2)
        tk.Label(row3, text="Priority:", bg=COLORS["card_bg"], fg=COLORS["text"], font=("Helvetica", 10)).pack(side="left", padx=5)
        ...
        tk.Label(row3, text="Deadline:", bg=COLORS["card_bg"], fg=COLORS["text"], font=("Helvetica", 10)).pack(side="left", padx=(15, 5))
        self.task_deadline_entry = tk.Entry(row3, font=("Helvetica", 11), fg=COLORS["text"], bg=COLORS["entry_bg"], width=12)
        ...
        # 点击任务切换完成状态
        self.task_list.bind("<Button-1>", self.on_task_click)
    ...
    def on_task_click(self, event):
        """点击任务行切换完成状态"""
        try:
            index = self.task_list.index("@%d,%d" % (event.x, event.y))
            line = self.task_list.get(index + " linestart", index + " lineend")
            if not line.strip():
                return
    ...
    def update_tasks_display(self):
        ...
        if not self.user.tasks:
            self.task_list.insert(tk.END, "No tasks yet. Add one above!")
            return
        ...
        for task in tasks:
            ...
            subject_str = f"({task['subject']})" if task.get("subject") else ""
            deadline_str = f"  📅 {task['deadline']}" if task.get("deadline") else ""
            
            line = f"{task['id']}. {status} {task['title']} {subject_str} {priority_icon}{deadline_str}\n"
            
            if task["completed"]:
                # 完成的任务：划掉
                self.task_list.insert(tk.END, line, "completed")
                self.task_list.tag_config("completed", overstrike=True, foreground=COLORS["text_light"])
            else:
    ...
    def setup_settings_tab(self):
        ...
        tk.Label(pw_card, text="Current Password:", bg=COLORS["card_bg"], fg=COLORS["text"], font=("Helvetica", 10)).pack(anchor="w", pady=(5, 0))
        ...
        tk.Label(pw_card, text="New Password (min 6):", bg=COLORS["card_bg"], fg=COLORS["text"], font=("Helvetica", 10)).pack(anchor="w", pady=(5, 0))
        ...
        tk.Label(pw_card, text="Confirm New Password:", bg=COLORS["card_bg"], fg=COLORS["text"], font=("Helvetica", 10)).pack(anchor="w", pady=(5, 0))
...
class TestStudyScheduler(unittest.TestCase):
    ...
    def test_add_task(self):
        ...
        self.user.add_task("Test task", "Math", "2026-08-15", "High")
