...
# ============================================
# DATA FILE PATH - 硬编码，确保路径正确
# ============================================
# 你的文件在 /Users/hueyyyy/Studymate.py/ 目录下
BASE_DIR = "/Users/hueyyyy/Studymate.py"
...
# ============================================
# DATA MANAGER - 直接函数，简单可靠
# ============================================

def load_users():
    """加载所有用户数据"""
    print(f"📁 Loading from: {USERS_FILE}")
    ...
    try:
        ...
        for username, user_data in data.items():
            print(f"   👤 {username}: {len(user_data.get('subjects', {}))} subjects")
...
def save_users(users):
    ...
    try:
        ...
        for username, user in users.items():
            ...
            print(f"   👤 {username}: {len(user.subjects)} subjects")
        with open(USERS_FILE, "w") as f:
            json.dump(data, f, indent=4)
        print(f"✅ Saved {len(data)} users")
        return True
    except Exception as e:
...
class AuthWindow:
    ...
    def do_login(self):
        ...
        print(f"🔍 User '{username}' has {len(user.subjects)} subjects")
...
class MainApp:
    ...
    def add_subject(self):
        ...
        try:
            ...
            print("=" * 50)
            print(f"📝 添加科目 '{name}' 后，当前 subjects 数量: {len(self.user.subjects)}")
            print(f"   subjects: {self.user.subjects}")
            print("=" * 50)
            
            # 保存到文件
            save_users(AuthWindow.users)
    ...
    def delete_account(self):
        if messagebox.askyesno("Confirm", "Delete your account permanently? This cannot be undone."):
            ...
            if username in AuthWindow.users:
