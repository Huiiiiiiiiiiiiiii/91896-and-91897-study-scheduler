...
class User:
    ...
    def generate_daily_plan(self, available_hours):
        ...
        if not (0 < available_hours <= MAX_HOURS_PER_DAY):
            raise ValueError(
                "Available study hours must be greater than 0 and no more than 24."
            )
        targets = {
    ...
    def generate_weekly_schedule(self, max_hours_per_day, days=7):
        ...
        if not (0 < max_hours_per_day <= MAX_HOURS_PER_DAY):
            raise ValueError(
                "Available hours per day must be greater than 0 and no more than 24."
            )
...
class MainApp:
    ...
    def setup_dashboard(self):
        ...
        tk.Label(
            add_card,
            text=(
                "Weekly target hours are the total you want to study in a 7-day period. "
                "Priority and nearby deadlines matter when available time is limited."
            ),
            bg=COLORS["card_bg"],
    ...
    def setup_plan_tab(self):
        ...
        explanation = tk.Label(
            frame,
            text=(
                "How allocation works: weekly target hours are totals for each 7-day period, "
                "not hours repeated every day. The weekly planner will not schedule "
                "more than those targets. Priority and deadlines decide which hours "
                "are protected when capacity is limited, and urgent sessions are placed "
    ...
    def setup_progress_tab(self):
        ...
        explanation = tk.Label(
            frame,
            text=(
                "Use this page after studying: record the time you actually completed. "
                "StudyMate compares this week's logged time with the desired weekly "
        ...
        self.progress_tree.column("Desired This Week", width=125, anchor="center")
        self.progress_tree.column("Logged This Week", width=120, anchor="center")
    ...
    def update_stats(self):
        ...
        completion = (total_logged / total_planned * 100) if total_planned else 0.0
...
class TestStudyScheduler(unittest.TestCase):
    ...
    def test_add_subject_valid_and_capitalised(self):
        ...
        saved_name = self.user.add_subject("digital technology", 5, "High", deadline, None, "A")
    ...
    def test_add_subject_rejects_past_deadline(self):
        past_deadline = (datetime.now() - timedelta(days=1)).strftime(DATE_FORMAT)
