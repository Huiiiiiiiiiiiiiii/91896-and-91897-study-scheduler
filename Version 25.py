...
# ===== 字体改成更自然的字体 =====
FONT_FAMILY = "Arial"  # Available on both macOS and Windows


def apply_cross_platform_button_style(parent):
    """Apply explicit button colours so labels remain visible on macOS and Windows."""
    for widget in parent.winfo_children():
        if isinstance(widget, tk.Button):
            widget.configure(
                ...
                disabledforeground="#c8c2d8",
                highlightthickness=0,
                bd=1,
            )
        apply_cross_platform_button_style(widget)
...
class User:
    ...
    def normalise_subject_name(name):
        """Return a consistently capitalised subject name with extra spaces removed."""
        words = name.strip().split()
        return " ".join(
            word if len(word) > 1 and word.isupper()
    ...
    def add_subject(
        ...
        priority="Medium",
        deadline=None,
        goal_hours=None,  # Kept only for compatibility with older versions.
        target_grade=None,
    ):
        raw_name = name.strip()
        if len(raw_name) < 2:
        ...
        # Treat Maths, maths and MATHS as the same subject.
        if any(
            existing_name.casefold() == normalised_name.casefold()
            for existing_name in self.subjects
        ):
            raise ValueError(
        ...
        if not (MIN_HOURS <= hours <= MAX_HOURS_PER_DAY):
            raise ValueError(
                f"Weekly hours must be between {MIN_HOURS} and "
                f"{MAX_HOURS_PER_DAY}."
            )
    ...
    def delete_subject(self, name):
        ...
        # Remove deleted subjects from study logs.
        for date in list(self.logs):
            self.logs[date].pop(name, None)
            if not self.logs[date]:
                del self.logs[date]

        # Keep existing tasks, but remove their link to the deleted subject.
    ...
    def get_total_logged_hours(self, subject_name):
        """Return the total study time logged for one subject across all dates."""
        return round(
            sum(day_log.get(subject_name, 0) for day_log in self.logs.values()),
            2,
        )
...
class AuthWindow:
    # FIX: 类变量在 __main__ 中被赋值，供所有实例共用
    users = {}

    def __init__(self):
        # FIX: 实例变量引用类变量，保持数据一致
    ...
    def show_login(self):
        ...
        apply_cross_platform_button_style(self.root)
...
class MainApp:
    ...
    def setup_dashboard(self):
        ...
        tk.Label(
            add_card, text="Subject Name:",
            bg=COLORS["card_bg"], fg=COLORS["text"]
        ).pack(anchor="w", pady=(5, 0))
        self.name_entry = tk.Entry(
            add_card, font=(FONT_FAMILY, 11),
            fg=COLORS["text"], bg=COLORS["entry_bg"]
        )
        self.name_entry.pack(fill="x", pady=(0, 10))

        tk.Label(
            add_card, text="Weekly Hours (0-24):",
            bg=COLORS["card_bg"], fg=COLORS["text"]
        ).pack(anchor="w", pady=(5, 0))
        self.hours_entry = tk.Spinbox(
            ...
            fg=COLORS["text"],
            bg=COLORS["entry_bg"],
            buttonbackground=COLORS["frame_bg"],
        )
        ...
        tk.Label(
            add_card, text="Deadline (DD/MM/YYYY, optional):",
            bg=COLORS["card_bg"], fg=COLORS["text"]
        ).pack(anchor="w", pady=(5, 0))
        ...
        tk.Label(
            add_card, text="Target Grade (optional):",
            bg=COLORS["card_bg"], fg=COLORS["text"]
        ).pack(anchor="w", pady=(5, 0))
        ...
        table_frame = tk.Frame(list_card, bg=COLORS["card_bg"])
        table_frame.pack(fill="both", expand=True)

        columns = ("Subject", "Weekly Hours", "Priority", "Deadline", "Target Grade")
        ...
        self.subject_tree.column("Deadline", width=115, anchor="center")
        self.subject_tree.column("Target Grade", width=100, anchor="center")

        subject_scrollbar = ttk.Scrollbar(
            table_frame, orient="vertical", command=self.subject_tree.yview
        )
        self.subject_tree.configure(yscrollcommand=subject_scrollbar.set)
        self.subject_tree.pack(side="left", fill="both", expand=True, padx=(5, 0), pady=5)
    ...
    def select_subject(self, event=None):
        ...
        self.selected_subject = values[0] if values else None
    ...
    def clear_all_subjects(self):
        ...
        if messagebox.askyesno("Confirm", "Delete ALL subjects?"):
            for subject_name in list(self.user.subjects):
    ...
    def setup_plan_tab(self):
        ...
        daily_row = tk.Frame(daily_card, bg=COLORS["card_bg"])
        daily_row.pack(fill="x", pady=5)

        tk.Label(
            daily_row, text="Hours per day:",
            bg=COLORS["card_bg"], fg=COLORS["text"],
            font=(FONT_FAMILY, 11),
        ).pack(side="left")
        self.avail_entry = tk.Spinbox(
            daily_row,
            from_=0.5,
            to=24,
            increment=0.5,
        ...
        tk.Label(
            control_frame, text="Hours per day:",
            bg=COLORS["card_bg"], fg=COLORS["text"],
            font=(FONT_FAMILY, 11),
        ).pack(side="left")
        self.weekly_avail_entry = tk.Spinbox(
            control_frame,
            from_=0.5,
            to=24,
            increment=0.5,
        ...
        self.weekly_avail_entry.pack(side="left", padx=(10, 15))
        self.weekly_avail_entry.delete(0, tk.END)
        self.weekly_avail_entry.insert(0, "3")
    ...
    def generate_today_plan(self):
        ...
        try:
            ...
            if available <= 0 or available > MAX_HOURS_PER_DAY:
                raise ValueError("Hours per day must be between 0.5 and 24.")
    ...
    def generate_weekly_plan(self):
        try:
            avail_str = self.weekly_avail_entry.get().strip()
    ...
    def export_report(self):
        try:
            ...
            # 优先保存到桌面，如果桌面不存在则使用当前目录
            from pathlib import Path
            desktop = Path.home() / "Desktop"
            if not desktop.exists():
                desktop = Path.cwd()  # 回退到当前工作目录
            filename = desktop / f"study_plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    def setup_progress_tab(self):
        ...
        stats_card = self.create_card(frame, "Statistics & Progress")
        ...
        chart_card = self.create_card(frame, "Weekly Study Distribution")
    ...
    def update_progress_tab(self):
        ...
        if subjects:
            current = self.log_subject_var.get()
    ...
    def log_study_time(self):
        ...
        try:
            ...
            if hours <= 0 or hours > MAX_HOURS_PER_DAY:
                raise ValueError("Logged hours must be between 0.1 and 24.")
    ...
    def update_stats(self):
        ...
        if not self.user.subjects:
            self.stats_display.insert(tk.END, "No subjects to show statistics for.")
            self.chart_canvas.delete("all")
            return

        total_hours = sum(data.get("hours", 0) for data in self.user.subjects.values())
        ...
        self.stats_display.insert(
            tk.END, f"Pomodoros completed: {self.user.pomodoro_count}\n\n"
        )

        self.stats_display.insert(tk.END, "--- Weekly Hours and Logged Study Time ---\n")
        for name, data in self.user.subjects.items():
            weekly_hours = data.get("hours", 0)
            logged_hours = self.user.get_total_logged_hours(name)
            grade = data.get("target_grade", "")
            grade_text = f" [Target: {grade}]" if grade else ""
            self.stats_display.insert(
                tk.END,
                f"{name}: {weekly_hours:g}h planned/week | "
                f"{logged_hours:g}h logged{grade_text}\n",
            )
...
class TestStudyScheduler(unittest.TestCase):
    ...
    def test_add_subject_valid(self):
        saved_name = self.user.add_subject(
            "math", 5, "High", "01/08/2026", None, "A"
        )
        self.assertEqual(saved_name, "Math")
        self.assertIn("Math", self.user.subjects)
        self.assertEqual(self.user.subjects["Math"]["hours"], 5)

    def test_subject_name_capitalisation(self):
        saved_name = self.user.add_subject("digital technology", 4)
        self.assertEqual(saved_name, "Digital Technology")

    def test_subject_name_preserves_acronym(self):
        saved_name = self.user.add_subject("NCEA english", 4)
        self.assertEqual(saved_name, "NCEA English")

    def test_duplicate_subject_case_insensitive(self):
        self.user.add_subject("Maths", 5)
        with self.assertRaises(ValueError):
            self.user.add_subject("maths", 3)
    ...
    def test_generate_plan_equal_weight(self):
    ...
    def test_generate_plan_uses_priority(self):
        self.user.add_subject("Math", 4, "High")
        self.user.add_subject("English", 4, "Medium")
        self.user.add_subject("Art", 4, "Low")
        plan = self.user.generate_plan(6, days=1)
        date = datetime.now().strftime(DATE_FORMAT)
        self.assertEqual(plan[date]["Math"], 3.0)
        self.assertEqual(plan[date]["English"], 2.0)
        self.assertEqual(plan[date]["Art"], 1.0)

    def test_deadline_increases_weight(self):
        urgent_date = (datetime.now() + timedelta(days=2)).strftime(DATE_FORMAT)
        self.user.add_subject("Math", 4, "Medium", urgent_date)
        self.user.add_subject("English", 4, "Medium")
        plan = self.user.generate_plan(4, days=1)
        date = datetime.now().strftime(DATE_FORMAT)
    ...
    def test_log_study_time(self):
        ...
        today = datetime.now().strftime(DATE_FORMAT)
        self.user.log_study_time(today, "Math", 1.5)
        self.assertEqual(self.user.get_total_logged_hours("Math"), 1.5)
