...
class User:
    ...
    def add_subject(self, name, hours, priority="Medium", deadline=None, goal_hours=None, target_grade=None):
        ...
        if any(existing.casefold() == normalised_name.casefold() for existing in self.subjects):
            raise ValueError(
                f"Subject '{normalised_name}' already exists. Please enter a different subject."
            )
        ...
        if goal_hours is not None and not (MIN_HOURS <= goal_hours <= MAX_HOURS_PER_DAY):
    ...
    def delete_subject(self, name):
        ...
        # Remove orphaned log entries and detach tasks from the deleted subject.
    ...
    def get_logged_hours_by_subject(self):
        totals = {name: 0.0 for name in self.subjects}
        for daily_log in self.logs.values():
    ...
    def log_study_time(self, date, subject, hours):
        ...
        if not (0 < hours <= MAX_HOURS_PER_DAY):
            raise ValueError("Logged hours must be greater than 0 and no more than 24.")
...
class MainApp:
    ...
    def setup_dashboard(self):
        ...
        tk.Label(
            add_card,
            text="Desired weekly study hours:",
            bg=COLORS["card_bg"],
            fg=COLORS["text"],
        ).pack(anchor="w", pady=(5, 0))
        self.hours_var = tk.StringVar(value="5")
        ...
        tk.Label(
            add_card,
            text=(
                "StudyMate uses weekly hours, priority and nearby deadlines "
        ...
        subject_columns = ("Subject", "Weekly Hours", "Priority", "Deadline", "Target Grade")
    ...
    def update_display(self):
        ...
        self.weekly_plan_display.config(fg=COLORS["text"], bg=COLORS["entry_bg"])
        self.stats_display.config(fg=COLORS["text"], bg=COLORS["entry_bg"])
        ...
        self.subject_tree.tag_configure("placeholder", foreground=COLORS["text_light"])
    ...
    def delete_subject(self):
        ...
        if not selected or not self.user.subjects:
            messagebox.showwarning("No Selection", "Select a subject from the table first.")
            return
        values = self.subject_tree.item(selected[0], "values")
        if not values or values[0] not in self.user.subjects:
            messagebox.showwarning("No Selection", "Select a valid subject first.")
            return
    ...
    def clear_all_subjects(self):
        if not self.user.subjects:
            ...
            return
        if messagebox.askyesno("Confirm", "Delete ALL subjects and their study logs?"):
            self.user.subjects = {}
            self.user.logs = {}
    ...
    def setup_plan_tab(self):
        ...
        explanation = tk.Label(
            frame,
            text=(
                "How allocation works: StudyMate considers desired weekly hours, "
                "subject priority and deadline urgency."
            ),
            font=(FONT_FAMILY, 10),
            bg=COLORS["bg"],
            fg=COLORS["text_light"],
        ...
        self.daily_hours_var = tk.StringVar(value="3")
        ...
        self.weekly_hours_var = tk.StringVar(value="3")
    ...
    def generate_weekly_plan(self):
        try:
            ...
            self.weekly_plan_display.insert(
                tk.END,
                f"{available:g} available hour(s) per day for {days} day(s)\n\n",
            )
    ...
    def setup_progress_tab(self):
        ...
        stats_card = self.create_card(frame, "Study Progress Summary")
        ...
        chart_card = self.create_card(frame, "Logged Study Distribution")
    ...
    def update_stats(self):
        ...
        if not self.user.subjects:
            self.stats_display.insert(tk.END, "No subjects to show progress for.")
            self.chart_canvas.delete("all")
            self.chart_canvas.create_text(
                220,
                75,
                text="Add a subject and log study time to see progress.",
                fill=COLORS["text_light"],
            )
            return

        logged_by_subject = self.user.get_logged_hours_by_subject()
        total_planned = sum(data["hours"] for data in self.user.subjects.values())
        total_logged = sum(logged_by_subject.values())

        self.stats_display.insert(tk.END, f"Desired weekly study hours: {total_planned:.1f}\n")
        self.stats_display.insert(tk.END, f"Total study time logged: {total_logged:.1f}\n")
        ...
        self.stats_display.insert(tk.END, "--- Planned vs Logged ---\n")

        for name, data in self.user.subjects.items():
            planned = float(data["hours"])
            logged = logged_by_subject.get(name, 0.0)
            grade = data.get("target_grade", "")
            grade_str = f" | Target grade: {grade}" if grade else ""
            self.stats_display.insert(
                tk.END,
                f"{name}: {planned:g}h desired/week | {logged:.1f}h logged{grade_str}\n",
            )

        self.draw_chart(logged_by_subject)

    def draw_chart(self, logged_by_subject):
        canvas = self.chart_canvas
        canvas.delete("all")

        if not logged_by_subject or max(logged_by_subject.values(), default=0) <= 0:
            canvas.create_text(
                250,
                75,
                text="No logged study time yet.",
                fill=COLORS["text_light"],
                font=(FONT_FAMILY, 10),
            )
            return
        ...
        items = list(logged_by_subject.items())
        num_items = len(items)
        max_width = max(width - 60, 100)
        bar_width = max(min(max_width / max(num_items, 1) - 10, 70), 20)
        max_bar_height = height - 50
        max_hours = max(logged_by_subject.values())

        x = 30
        for name, hours in items:
            bar_height = (hours / max_hours) * max_bar_height if max_hours else 0
    ...
    def setup_tasks_tab(self):
        ...
        tk.Label(
            stats_row,
            text="Select a task, then use the buttons below (or double-click to change status).",
            font=(FONT_FAMILY, 9),
            bg=COLORS["card_bg"],
            fg=COLORS["text_light"],
        ).pack(side="right")
    ...
    def update_tasks_display(self):
        ...
        if not self.user.tasks:
            self.task_tree.insert("", "end", text="0", values=("-", "-", "No tasks yet", "-"), tags=("placeholder",))
            self.task_tree.tag_configure("placeholder", foreground=COLORS["text_light"])
            return
...
class TestStudyScheduler(unittest.TestCase):
    ...
    def test_add_subject_valid_and_capitalised(self):
        saved_name = self.user.add_subject("digital technology", 5, "High", "01/09/2026", None, "A")
    ...
    def test_priority_weighted_plan(self):
        ...
        date = datetime.now().strftime(DATE_FORMAT)
        self.assertEqual(plan[date], {"Maths": 3.0, "English": 2.0, "Art": 1.0})
