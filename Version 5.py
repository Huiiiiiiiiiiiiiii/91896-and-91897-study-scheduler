...
class MainApp:
    ...
    def setup_dashboard(self):
        ...
        # Subject name
        tk.Label(add_card, text="Subject Name:", bg=COLORS["frame_bg"], fg=COLORS["text"]).pack(anchor="w", pady=(5, 0))
        self.name_entry = tk.Entry(add_card, font=("Helvetica", 11))
        self.name_entry.pack(fill="x", pady=(0, 10))
        
        # Hours
        tk.Label(add_card, text="Weekly Hours (0-24):", bg=COLORS["frame_bg"], fg=COLORS["text"]).pack(anchor="w", pady=(5, 0))
        self.hours_entry = tk.Entry(add_card, font=("Helvetica", 11))
        self.hours_entry.pack(fill="x", pady=(0, 10))
        
        # Priority
        tk.Label(add_card, text="Priority:", bg=COLORS["frame_bg"], fg=COLORS["text"]).pack(anchor="w", pady=(5, 0))
        ...
        # Deadline
        tk.Label(add_card, text="Deadline (YYYY-MM-DD, optional):", bg=COLORS["frame_bg"], fg=COLORS["text"]).pack(anchor="w", pady=(5, 0))
        ...
        tk.Label(row, text="Hours per day:", bg=COLORS["frame_bg"], fg=COLORS["text"], font=("Helvetica", 11)).pack(side="left")
    ...
    def update_display(self):
        ...
        for name, data in self.user.subjects.items():
            priority_emoji = {"High": "High", "Medium": "Med", "Low": "Low"}.get(data["priority"], "")
            deadline_str = f"  Due: {data['deadline']}" if data.get("deadline") else ""
            self.subject_list.insert(
                tk.END,
                f"{name}  {data['hours']}h/week  [{data['priority']}]{deadline_str}\n"
            )
    ...
    def setup_progress_tab(self):
        ...
        tk.Label(row, text="Date:", bg=COLORS["frame_bg"], fg=COLORS["text"], font=("Helvetica", 11)).pack(side="left")
        ...
        tk.Label(row, text="Subject:", bg=COLORS["frame_bg"], fg=COLORS["text"], font=("Helvetica", 11)).pack(side="left", padx=(10, 0))
        ...
        tk.Label(row, text="Hours:", bg=COLORS["frame_bg"], fg=COLORS["text"], font=("Helvetica", 11)).pack(side="left", padx=(10, 0))
    ...
    def setup_settings_tab(self):
        ...
        tk.Label(pw_card, text="Current Password:", bg=COLORS["frame_bg"], fg=COLORS["text"]).pack(anchor="w", pady=(5, 0))
        self.old_pw_entry = tk.Entry(pw_card, show="●", font=("Helvetica", 11))
        self.old_pw_entry.pack(fill="x", pady=(0, 10))
        
        tk.Label(pw_card, text="New Password (min 6):", bg=COLORS["frame_bg"], fg=COLORS["text"]).pack(anchor="w", pady=(5, 0))
        self.new_pw_entry = tk.Entry(pw_card, show="●", font=("Helvetica", 11))
        self.new_pw_entry.pack(fill="x", pady=(0, 10))
        
        tk.Label(pw_card, text="Confirm New Password:", bg=COLORS["frame_bg"], fg=COLORS["text"]).pack(anchor="w", pady=(5, 0))
