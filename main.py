...
REPORT_SEPARATOR_WIDTH = 58
REPORT_TIMESTAMP_FORMAT = "%Y%m%d_%H%M%S"
REPORT_DISPLAY_TIME_FORMAT = "%Y-%m-%d %H:%M"
...
TEMP_USERS_FILE = f"{USERS_FILE}.tmp"
...
def validate_study_log_date(date_text):
    """Return a valid study-log date and reject dates in the future."""
    try:
        log_date = datetime.strptime(date_text, DATE_FORMAT).date()
    except (TypeError, ValueError) as exc:
        raise ValueError("Study date must use DD/MM/YYYY.") from exc
...
class User:
    ...
    def add_subject(
        ...
        if goal_hours is not None and not (
            ...
            raise ValueError(
                f"Goal hours must be between {MIN_HOURS} "
                f"and {MAX_HOURS_PER_DAY}."
            )
    ...
    def generate_daily_plan(self, available_hours):
        ...
        if not (0 < available_hours <= MAX_HOURS_PER_DAY):
            raise ValueError(
                ...
                f"and no more than {MAX_HOURS_PER_DAY}."
            )
        targets = {
    ...
    def generate_weekly_schedule(self, max_hours_per_day, days=7):
        ...
        if days < 1 or days > MAX_PLAN_DAYS:
            raise ValueError(
                f"Days must be between 1 and {MAX_PLAN_DAYS}."
            )
...
class DataManager:
    ...
    def save_users(users):
        """Save user data atomically and report file-system errors."""
        try:
            ...
            os.replace(TEMP_USERS_FILE, USERS_FILE)
            return True
...
class MainApp:
    ...
    def export_report(self):
        ...
        try:
            ...
            filename = desktop / (
                "study_plan_"
                f"{datetime.now().strftime(REPORT_TIMESTAMP_FORMAT)}.txt"
            )
            ...
            with open(filename, "w", encoding="utf-8") as f:
                ...
                f.write(
                    "Generated: "
                    f"{datetime.now().strftime(REPORT_DISPLAY_TIME_FORMAT)}\n"
                )
                f.write("=" * 58 + "\n\n")
                f.write(
                    f"Period: {days} day(s)\n"
...
class TestStudyScheduler(unittest.TestCase):
    ...
    def test_logged_hours_by_subject(self):
        ...
        self.user.log_study_time(yesterday, "Maths", 1.5)
        self.user.log_study_time(today, "Maths", 0.5)
    ...
    def test_parse_number_rejects_alphabetic_input(self):
        """Verify alphabetic numeric input receives a validation error."""
        with self.assertRaisesRegex(ValueError, "must be a number"):
            parse_number("abc", "Hours")

    def test_parse_number_rejects_blank_input(self):
        """Verify blank required numeric input is rejected."""
        with self.assertRaisesRegex(ValueError, "is required"):
            parse_number("", "Hours")

    def test_parse_number_rejects_non_finite_values(self):
        """Verify NaN and infinity cannot enter scheduling calculations."""
        for value in ("nan", "inf", "-inf"):
            with self.subTest(value=value):
                with self.assertRaisesRegex(ValueError, "finite number"):
                    parse_number(value, "Hours")

    def test_subject_hours_accept_boundaries(self):
        """Verify zero and 24 are accepted subject-hour boundaries."""
        self.user.add_subject("Art", 0)
        self.user.add_subject("Maths", MAX_HOURS_PER_DAY)
        self.assertEqual(self.user.subjects["Art"]["hours"], 0)
        self.assertEqual(
            self.user.subjects["Maths"]["hours"],
            MAX_HOURS_PER_DAY,
        )

    def test_weekly_plan_day_boundaries(self):
        """Verify one and 30 days are accepted and 31 is rejected."""
        self.user.add_subject("Maths", 2)
        self.user.generate_weekly_schedule(1, days=1)
        self.user.generate_weekly_schedule(1, days=MAX_PLAN_DAYS)
        with self.assertRaises(ValueError):
            self.user.generate_weekly_schedule(1, days=MAX_PLAN_DAYS + 1)

    def test_log_time_rejects_future_date(self):
        """Verify actual study time cannot be logged in the future."""
        self.user.add_subject("Maths", 5)
        future_date = (
            datetime.now() + timedelta(days=1)
        ).strftime(DATE_FORMAT)
        with self.assertRaisesRegex(ValueError, "cannot be in the future"):
            self.user.log_study_time(future_date, "Maths", 1)

    def test_log_time_hour_boundaries(self):
        """Verify logged hours accept 24 but reject zero and above 24."""
        self.user.add_subject("Maths", 5)
        today = datetime.now().strftime(DATE_FORMAT)
        self.user.log_study_time(today, "Maths", MAX_HOURS_PER_DAY)
        with self.assertRaises(ValueError):
            self.user.log_study_time(today, "Maths", 0)
        with self.assertRaises(ValueError):
            self.user.log_study_time(today, "Maths", MAX_HOURS_PER_DAY + 1)

    def test_task_rejects_blank_title_and_invalid_priority(self):
        """Verify invalid task title and priority values are rejected."""
        with self.assertRaises(ValueError):
            self.user.add_task("   ")
        with self.assertRaisesRegex(ValueError, "Invalid task priority"):
            self.user.add_task("Revise", priority="Urgent")

    def test_task_rejects_past_deadline(self):
        """Verify task deadlines in the past are rejected."""
        past_deadline = (
            datetime.now() - timedelta(days=1)
        ).strftime(DATE_FORMAT)
        with self.assertRaisesRegex(ValueError, "cannot be in the past"):
            self.user.add_task("Revise", deadline=past_deadline)
