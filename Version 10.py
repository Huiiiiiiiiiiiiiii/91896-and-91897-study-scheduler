...
class MainApp:
    ...
    # ==========================================
    # DASHBOARD TAB - 新增 Deadline 提醒区域
    # ==========================================
    def setup_dashboard(self):
        frame = self.dashboard_frame
        
        # ===== 新增：Deadline 提醒区域（放在最顶部） =====
        self.deadline_alert_frame = tk.Frame(frame, bg=COLORS["bg"])
        self.deadline_alert_frame.pack(fill="x", padx=10, pady=(10, 0))
        
        self.deadline_alert = tk.Label(
            self.deadline_alert_frame,
            text="",
            font=("Helvetica", 10),
            bg=COLORS["bg"],
            fg=COLORS["delete"],
    ...
    def update_deadline_alert(self):
        """显示即将到来的截止日期提醒"""
        if not self.user.subjects:
            self.deadline_alert.config(text="", bg=COLORS["bg"])
            return
        ...
        for name, data in self.user.subjects.items():
            if data.get("deadline"):
                try:
                    ...
                    if 0 <= days_left <= 7:  # 未来7天内的截止日期
                        upcoming.append((name, days_left, data["priority"]))
                    elif days_left < 0:
                        upcoming.append((name, -1, data["priority"]))  # 已过期
                except ValueError:
                    pass
        
        if not upcoming:
            self.deadline_alert.config(text="✅ No upcoming deadlines in the next 7 days.", 
        ...
        messages = []
        for name, days_left, priority in upcoming:
            if days_left < 0:
        ...
        self.deadline_alert.config(text=alert_text, fg=COLORS["delete"], bg=COLORS["bg"])
    ...
    def generate_today_plan(self):
        ...
        try:
            ...
            for name, hours in plan.get(today, {}).items():
                ...
                # ===== 检查该科目是否有截止日期 =====
                deadline_warning = ""
                if name in self.user.subjects:
                    data = self.user.subjects[name]
                    if data.get("deadline"):
                        try:
                            deadline_date = datetime.strptime(data["deadline"], DATE_FORMAT)
                            days_left = (deadline_date - datetime.now()).days
                            if days_left < 0:
                                deadline_warning = " ⚠️ OVERDUE!"
                            elif days_left <= 3:
                                deadline_warning = f" 🔴 DUE IN {days_left} DAYS!"
                            elif days_left <= 7:
                                deadline_warning = f" 🟡 Due in {days_left} days"
                        except ValueError:
                            pass
                
                self.plan_display.insert(tk.END, f"  {name}{deadline_warning}\n")
                self.plan_display.insert(tk.END, f"  {bar} {hours} hour(s)\n\n")
            
            self.plan_display.insert(tk.END, "=" * 50 + "\n")
            self.plan_display.insert(tk.END, "  💡 Subjects with deadlines get extra time")
        except ValueError as e:
            messagebox.showerror("Error", str(e))
        ...
        try:
            ...
            for date, daily_plan in plan.items():
                ...
                for name, hours in daily_plan.items():
                    ...
                    self.weekly_plan_display.insert(tk.END, f"   {name}: {bar} {hours}h{deadline_mark}\n")
